# G3 expected-case prompt (Phase 3, G3 correction round 2) — v4

v4 = v3 with one change made for G3 correction round 2 (re-audit Phase3_G3_reaudit_Agent2.md): where metres that
a charge's depths do not place can be placed in many ways, the output carries the number of ways and, if there are
more than 25, only the two extreme ways (`allocation_count`, `tolerance:lowest` / `tolerance:highest`). The rules
the reader applies are unchanged. v1-v3 are unchanged and remain the prompts of the earlier readers. v4 is used for
the round-2 cases only.
...
For each case, write ONE line of JSON to {OUTPUT} as soon as the case is done, in packet order:
    ...
    {"id": "...",
     ...
     "allocation_count": <the number of ways of placing metres the charge's depths do not place (see "tolerance"
                          below); null if there is nothing to place>,
...
- "tolerance:<a few words>" - one per way of placing metres the charge's depths do not place. Count the ways in
  steps of the finest decimal place used by the charged quantity and the depths (whole metres when all of them are
  whole) and give the count in `allocation_count`. If there are more than 25 ways, give only the two ways with the
  lowest and the highest amount, labelled "tolerance:lowest" and "tolerance:highest" (every way between them is
  then one you did not list); otherwise give every way.
