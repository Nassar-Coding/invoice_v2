# G3 expected-case prompt (Phase 3, G3) — v1

Used for six independent subagent readers: civil synthetic ×2, drilling synthetic ×2, civil real lines, drilling
real lines. The packets are built by `tools/g3_cases.py` and carry inputs only.

The readers compute expected results from the contract scans before and without the G3 pricing code. Their
output is compared field by field with the implementation by `tools/verify_g3.py`. Every disagreement is settled
against the scan, and the settlement is recorded.
